window.onload = () => {
  if (document.getElementById('_message')) {
    alrt(document.getElementById('_message').value);
  }

  document.getElementById('slide_btn').onclick = () => {
    document.getElementById('slide').classList.add('info_show');
    document.getElementById('blind').classList.add('blind_abled');
    return false;
  };

  document.getElementById('main_search').onclick = () => {
    const link = document.getElementById('main_search');
    const searchText = document.getElementById('searchtext');

    if (chk_name(searchText)) {
      link.setAttribute('href', link.getAttribute('href') + `&searchtext=${searchText.value}`);
      return true;
    }
    alert('검색어를 입력해 주세요.');
    return false;
...
function slide_disabled() {
  document.getElementById('slide').classList.remove('info_show');
  document.getElementById('blind').classList.remove('blind_abled');
}
